...
{
    [Fact]
    public void WeatherRetrieverThrowsException_ExceptionCaughtAndNonOkStatusWeatherResponseReturned()
    {
        ...
        var service = new WeatherService(retriever);
        var output = service.GetWeather(new WeatherRequest {Location = "Test"});
    ...
    [Fact]
    public void WeatherRetrieverReturnsValidResponse_WeatherResponseWithOkStatusAndWeatherObjectReturned()
    {
        var options = new WeatherOptions() { ApiKey = "Key" };
        var mockOptions = new Mock<IOptions<WeatherOptions>>();
        mockOptions.Setup(x => x.Value).Returns(options);
